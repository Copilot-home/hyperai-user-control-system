...
export function validateDurableEvent(event = {}) {
  if (!event.id || !event.aggregate_id || !Number.isInteger(event.sequence) || !event.event_type) throw new Error('DURABLE_EVENT_IDENTITY_INVALID');
  if (!event.event_hash || !event.schema_version || !event.created_at || !event.correlation_id) throw new Error('DURABLE_EVENT_PROVENANCE_INVALID');
